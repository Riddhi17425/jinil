    ...
    <div class="container-fluid">
        <div class="industries_details">
            <div class="mb-3 mb-lg-0">
                <img class="w-100" src="{{ asset('public/Product/detail_image/' . $product->detail_image) }}"
            ...
            <div>
                ...
                <button class="com_btn product-enquire-btn" data-product="{{ $product->name }}" data-bs-toggle="modal"
                    data-bs-target="#productEnquiryModal">
                    Enquire Now
                </button>
            </div>
        ...
        <div class="container">
            <p class="title_34 text-105 d-flex justify-content-center text-center lh-sm mb-0">
                ...
                <span class="d-none d-lg-inline">
                    <svg width="40" height="23" viewBox="0 0 57 40" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
...
@if (isset($product->blast_wheels) &&
        is_countable($product->blast_wheels) &&
        count($product->blast_wheels) > 0 &&
        isset($product->blast_wheels_image) &&
        $product->blast_wheels_image != '')
    <section class="mt_80">
        <div class="container-fluid">
            <div class="industries_details">
...
@if (isset($product->tech_specifications) &&
        is_countable($product->tech_specifications) &&
        count($product->tech_specifications) > 0)
    <section class="mt_100">
        <div class="container">
            <div class="sec_hed_top mb_40">
...
@elseif(isset($product->technical_details) && $product->technical_details != '')
<section class="mt_100">
    <div class="container">
        <div class="sec_hed_top mb_40">
            <p>Engineered Output Parameters</p>
            <h2 class="title_60">Technical Details</h2>
        </div>
...
@if (isset($product->configuration_title) &&
        $product->configuration_title != '' &&
        isset($product->configuration_description) &&
        $product->configuration_description != '')
    <section class="mt_100">
        <div class="container">
            <div class="sec_hed_top mb_40">
...
@if (isset($product->why_choose_title) &&
        $product->why_choose_title != '' &&
        isset($product->why_choose_description) &&
        $product->why_choose_description != '')
    <section class="mt_100">
        <div class="container">
            <div class="sec_hed_top mb_40">
            ...
            <div class="industry_section">
                <div class="row justify-content-between mb_40">
                    <div class="col-lg-10">
                        ...
                        <div class="mb_40 text-white">{!! $product->related_industries_desc ??
                            "Explore a wide range of industries where shot blasting plays a critical role in improving surface quality, durability, and production efficiency. From automotive and foundry to fabrication and heavy engineering, Jinil's solutions are designed to meet diverse application needs, ensuring reliable surface preparation, consistent finishing, and optimized performance across demanding industrial environments." !!}</div>
                        <div class="slider_arrow mt-4 mt-lg-0 col">
                            <svg class="prev_arrow" width="68" height="68" viewBox="0 0 68 68"
                                fill="none" xmlns="http://www.w3.org/2000/svg">
                                <rect x="68" y="68" width="68" height="68" rx="34"
                                    transform="rotate(180 68 68)" fill="#E4ECF4" />
                                <path
                            ...
                            <svg class="next_arrow" width="68" height="68" viewBox="0 0 68 68"
                                fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <div class="industry_item_wrapper mx-3">
                                <a href="{{ route('industry', $v->url) }}">
                                    <div class="industry_item"
                                        style="background-image: url({{ asset('public/indcategory/icon_image/' . $v->icon_image) }})">
                                    </div>
...
@php
    ...
    if (
        isset($product->design_features) &&
        is_countable($product->design_features) &&
        count($product->design_features) > 0
    ) {
        $sections[] = 'design';
    }
    if (
        isset($product->selection_guidelines) &&
        is_countable($product->selection_guidelines) &&
        count($product->selection_guidelines) > 0
    ) {
        $sections[] = 'selection';
    }
    if (
        isset($product->optional_features) &&
        is_countable($product->optional_features) &&
        count($product->optional_features) > 0
    ) {
        $sections[] = 'optional';
    }
    ...
    <div class="container">
        <div class="service_Scope_card">
            <div class="row">
                ...
                @endif
                @if (isset($product->selection_guidelines) &&
                        is_countable($product->selection_guidelines) &&
                        count($product->selection_guidelines) > 0)
                    <div class="<?php echo $colClass; ?>">
                        <div class="service_Scope py-4">
                ...
                @endif
                @if (isset($product->optional_features) &&
                        is_countable($product->optional_features) &&
                        count($product->optional_features) > 0)
                    <div class="<?php echo $colClass; ?>">
                        <div class="service_Scope py-4">
...
@if (isset($product->operational_accessories) &&
        is_countable($product->operational_accessories) &&
        count($product->operational_accessories) > 0)
    <section class="mt_100">
        <div class="container">
            <div class="sec_hed_top mb_40">
                <h2 class="title_60">Optional Accessories</h2>
                <div class="text-585 d-block">{!! $product->optional_accessories_desc ??
                    'The machine can be upgraded with accessories based on production needs and application requirements.' !!}</div>
    ...
    <section class="mb_100 mt_100">
        <div class="container">
            <div class="sec_hed_top mb_40">
                ...
                <div class="text-585 d-block">{!! $product->faqs_desc ??
                    "Clear answers to common questions about Jinil's shot blasting machines, surface preparation solutions, and engineering support—helping you make informed decisions with confidence." !!}</div>
            </div>
