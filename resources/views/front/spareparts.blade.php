@include('layouts.frontheader')

<section class="navi_page">
    <div class="container-fluid">

        <div class="navi_page_child">
            <div>
                <p class="title_24"><a href="{{ url('/') }}" class="text-585">Home /</a> <a
                        href="{{ route('spareparts') }}" class="text-585">Spare Parts</a> </p>
                <h2 class="title_60">Spare Parts</h2>
                <p class="mb-0">High-performance spare parts designed for reliable operation, reduced downtime, and consistent blasting efficiency across industrial applications. Manufactured for durability and precision, these components support smooth machine performance, extended service life, and optimized surface preparation results.</p>
            </div>

            <a href="{{ route('contact') }}" class="contact_circle">

                <!-- circular text image -->
                <img src="{{ asset('public/front/images/innder-header-jump.svg') }}" class="circle_text_img">

                <svg class="arrow_img" width="18" height="23" viewBox="0 0 18 23" fill="none"
                    xmlns="http://www.w3.org/2000/svg">
                    <path
                        d="M8.85653 1.15617L8.85653 20.9552L8.85653 1.15617ZM8.85653 20.9552L16.5562 13.2555L8.85653 20.9552ZM8.85653 20.9552L1.15692 13.2556L8.85653 20.9552Z"
                        fill="#58595B" />
                    <path
                        d="M8.85653 1.15617L8.85653 20.9552M8.85653 20.9552L16.5562 13.2555M8.85653 20.9552L1.15692 13.2556"
                        stroke="#58595B" stroke-width="2.31318" stroke-linecap="round" stroke-linejoin="round" />
                </svg>

            </a>
        </div>

    </div>
</section>



<section class="faq_main mt_80 mb_100">
    <div class="container-fluid">

        <!-- Tabs -->
        <div class="faq_tabs mb_60 justify-content-center">

            <button class="com_btn active" data-tab="all">
                All
            </button>

            @foreach ($categories as $category)

                <button class="com_btn"
                    data-tab="{{ $category->url }}">

                    {{ $category->category }}

                </button>

            @endforeach

        </div>

        <!-- Spare Parts -->
        <div class="row gy-5" id="sparePartsWrapper">

            @forelse($spareparts as $sparepart)

                <div class="col-md-6 col-lg-4 sparepart-item"
                    data-category="{{ $sparepart->category_url }}">

                    <div class="fea_mac">

                        <div class="fea_mac_img">
                            <img class="img-fluid"
                                src="{{ asset('public/spareparts/'.$sparepart->image) }}"
                                alt="{{ $sparepart->title }}">
                        </div>

                        <div class="fea_mac_content">
                            <div class="fea_mac_content_inner">

                                <h3 class="title_24">
                                    {{ $sparepart->title }}
                                </h3>

                            </div>
                        </div>

                    </div>

                </div>

            @empty

                <div class="col-12">
                    <p class="text-center">
                        No spare parts available at the moment.
                    </p>
                </div>

            @endforelse

        </div>

    </div>
</section>

<script>

document.addEventListener("DOMContentLoaded", function () {

    const tabs = document.querySelectorAll(".faq_tabs .com_btn");
    const items = document.querySelectorAll(".sparepart-item");

    tabs.forEach(tab => {

        tab.addEventListener("click", function () {

            // Remove active class
            tabs.forEach(t => t.classList.remove("active"));

            // Add active class
            this.classList.add("active");

            const target = this.dataset.tab;

            items.forEach(item => {

                if (target === "all") {

                    item.style.display = "block";

                } else {

                    if (item.dataset.category === target) {
                        item.style.display = "block";
                    } else {
                        item.style.display = "none";
                    }

                }

            });

        });

    });

});
</script>


@include('front.serviceform')

@include('layouts.frontfooter')